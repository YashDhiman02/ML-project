...
class classification():
    def __init__(self,path="D:/IISER-B/IISERB Courses/5th Sem/ECS-308_DSML/Project/Phase_2",clf_opt='lr',no_of_selected_features=None):
        self.path = path
...
#            clf = AdaBoostClassifier(algorithm='SAMME',n_estimators=100)            
    ...
    # Random Forest 
            ...
            'clf__n_estimators':(30,50,80,100,150),
            'clf__max_depth':(15,29,50,100,200,300),
            'clf__max_features':('auto', 'sqrt', 'log2'),
            }          
...
# Load the data 
    ...
    # Load the file using Pandas       
        ...
        check_data = pd.read_csv("training_data.csv")
        print('Checking missing instances')
        print(check_data.isnull().sum())


        reader.drop(['Time', 'Amount'],axis=1,inplace=True)
        labels=target['Class']

        self.get_class_statistics(labels)          
        sc = MinMaxScaler()
        actual = sc.fit_transform(reader)
...
# Experiments using training data only during training phase (dividing it into training and validation set)
        ...
        for train_index, test_index in skf.split(data,labels):
            ...
            grid = GridSearchCV(pipeline,clf_parameters,scoring='f1_macro',cv=10,n_jobs=-1)
    ...
    # Evaluation
        ...
        print ('\n Confusion Matrix:\n')
        cm = pd.DataFrame(
        confusion_matrix(actual_class_labels, predicted_class_labels,labels=None), 
        index=['true label: True Transaction', 'true label: Fraud Transaction'], 
        columns=['predicted label: True Transaction', 'predicted label: Fraud Transaction']
        )
        print(cm)
